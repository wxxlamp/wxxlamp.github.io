...
description: "作者详细记录了拼多多拼越计划的两轮面试经历。一面考察SQL子查询聚合、聚簇索引与回表原理、联合索引最左原则、分布式服务调用优化、CAS与悲观锁对比等基础知识；二面深入考察RPC序列化原理、TCP脏连接、Java BIO/NIO/AIO模型、代理模式缺陷、反射性能瓶颈、幻读产生原因、Redis跳表结构等进阶话题。面试整体偏基础但有深度，最终因待遇为白菜价而拒绝offer。"
---
