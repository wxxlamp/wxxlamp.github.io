...
1.  **Dependencies**: The skill requires Python 3 and the `requests` library.
    ```bash
    pip install -r .Codex/skills/img-uploader/requirements.txt
    ```
2.  **Configuration**: An API token or client ID is required depending on the provider.
    ...
    *   **Config File**: `.Codex/skills/img-uploader/config.json`
        ```json
...
```bash
python3 .Codex/skills/img-uploader/image-uploader.py <path_to_image>
```
...
```bash
python3 .Codex/skills/img-uploader/image-uploader.py /Users/me/Pictures/screenshot.png
```

**Upload to sm.ms:**
```bash
python3 .Codex/skills/img-uploader/image-uploader.py /Users/me/Pictures/screenshot.png --provider smms
```

**Upload to GitHub (jsDelivr CDN):**
```bash
python3 .Codex/skills/img-uploader/image-uploader.py /Users/me/Pictures/screenshot.png --provider github
```

**Upload with explicit Client ID:**
```bash
python3 .Codex/skills/img-uploader/image-uploader.py image.png --token "YOUR_IMGUR_CLIENT_ID"
```

**Upload to Imgur using env var:**
```bash
IMGUR_CLIENT_ID="your_id" python3 .Codex/skills/img-uploader/image-uploader.py image.png
```

**Upload to GitHub using env vars:**
```bash
IMAGE_UPLOADER_GITHUB_TOKEN="your_token" IMAGE_UPLOADER_GITHUB_OWNER="user" IMAGE_UPLOADER_GITHUB_REPO="images" python3 .Codex/skills/img-uploader/image-uploader.py image.png --provider github
```
