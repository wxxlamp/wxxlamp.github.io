...
- 截图译图仍须保持原裁剪、红框/箭头、数值、时间、学校/机构名、按钮状态与界面关系。翻译不等于重设计，不补全裁掉的画面；图注标明为译图并链接原截图。长文本逐张处理、复审通过后再做下一张，避免一次提交大量图片后集中返工。
...
4. 工作区 `source/_data/references.json`（配置键 `reference_catalog`）可记录 `site_aliases`、`route_aliases`、`pairs: [{"zh": "...", "en": "..."}]` 、`page_pairs: [{"zh": "/", "en": "/en/"}]` 与 `original_sources: [{"url": "...", "language": "zh"}]`。`page_pairs` 用于首页、关于等非文章页面的已核验路径；未知页面不猜 `/en/`，也不为通过校验向英文正文塞入多余中文链接。只登记已经检查过的对应关系，不根据网址模式猜测。
