...
description: "通过复现实验揭示HashMap在并发put场景下的两类线程安全问题。第一类：扩容时新数组先赋给table，其他线程写入新元素后，老线程继续复制老桶数据会将新插入的值覆盖，导致元素丢失且size与实际数量不符；第二类：两线程同时判断同一桶为null后并发插入，后执行线程覆盖先执行线程的值。同时澄清1.8已修复并发死循环问题，给出ConcurrentHashMap等替代方案。"
---
