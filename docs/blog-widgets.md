...
2026-09-10 的待发布改动按用户最新选择：自建 Umami、Umami Cloud、
不蒜子同时独立采集；文章与页脚只展示自建 Umami 的累计 PV/UV。
Cloud 继续提供独立后台，不蒜子只采集、不参与展示或故障回退。
汇总接口只开放固定博客的数字，缓存约一分钟；失败或 10 秒超时显示
“统计暂不可用”，有效的 0 正常展示。先发布自建汇总接口，再发布博客配置。
