...
图床选择顺序为本次用户明确选择、环境变量、已有工作区配置、最后才是默认值。先读取配置再判断缺什么；GitHub 等已配置且已获授权的目标继续使用，不因插件默认 Imgur 再询问一次，也不改用其他图床。用户改变提供商时记录在工作区配置/项目偏好，不能只停留在对话中。
...
`github_cdn` 可选 `jsdelivr` 或 `china`。GitHub 上传按内容校验复用已有文件，返回固定提交 SHA 的地址及本地 SHA-256；同名不同内容停止覆盖。批量上传串行执行（避免分支写冲突），逐张持久化成功结果；失败恢复只补缺项。链接生成不等于 CDN 验证成功，仍需检查可访问性与当前已审图片的一致性。
