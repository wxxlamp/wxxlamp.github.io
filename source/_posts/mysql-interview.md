...
description: "系统整理MySQL高频面试题，覆盖数据库核心知识的四大模块：ACID事务特性、隔离级别与脏读幻读、三大范式与反范式权衡；B+树/Hash/全文索引原理、聚簇索引与回表、最左前缀与索引下推优化；InnoDB与MyISAM存储引擎对比、行锁/间隙锁/MVCC并发控制机制；以及一条SQL从连接器到执行器的完整执行链路与redo log/binlog的区别，适合Java后端面试备战。"
---
