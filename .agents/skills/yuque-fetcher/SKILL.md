...
1.  **Dependencies**: Python 3, Playwright, and requests.
    ```bash
    pip install -r .Codex/skills/yuque-fetcher/requirements.txt
    playwright install chromium
    ```
2.  **img-uploader skill**: Must be configured if you want images re-hosted (see `.Codex/skills/img-uploader/SKILL.md`). Use `--no-images` to skip image processing.
...
```bash
python3 .Codex/skills/yuque-fetcher/yuque-fetcher.py <url> --login
```

## Usage

```bash
python3 .Codex/skills/yuque-fetcher/yuque-fetcher.py <yuque_url> [-o output_path] [--no-images] [--login]
```
...
```bash
python3 .Codex/skills/yuque-fetcher/yuque-fetcher.py https://www.yuque.com/wangxingxing-f4sey/life/ka2yr80x894tbtgt
```

**Fetch with custom output path:**
```bash
python3 .Codex/skills/yuque-fetcher/yuque-fetcher.py https://www.yuque.com/user/repo/slug -o ~/notes/article.md
```

**Fetch without image processing:**
```bash
python3 .Codex/skills/yuque-fetcher/yuque-fetcher.py https://www.yuque.com/user/repo/slug --no-images
```
