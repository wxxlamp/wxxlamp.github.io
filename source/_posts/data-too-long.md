...
description: "记录实习中遭遇Oracle varchar2字段4000字节上限导致数据插入失败的问题，对比更改字段类型、建立外部索引、拆分字段三种方案的优劣，最终采用新建EXT溢出扩展表并以链表结构串联分片数据的方案，通过代理或拦截器实现对原有insert与query逻辑的最小侵入，同时探讨了按Oracle编码方式切割与直接按字节存储两种确定MAX_VALUE的方法。"
---
