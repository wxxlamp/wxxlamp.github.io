...
- 语气：先与 raw/source.md 比较人称、直接程度、情绪、幽默和节奏，再参考历史档案。写出具体保留的原稿片段和位置；对明显重写的段落说明为何需要改，不能仅写“更专业、更克制”。保留原稿真实细节，不能从历史文章借经历填进新稿。
- 自然表达：开头是否进入具体事，句段是否同节奏，结尾是否强行升华，标题是否夸张，互动是否像群发模板？不要机械删除“其实”“所以”等作者常用承接词，也不要为了不整齐刻意制造口误。检查编辑旁白、重复限定、把事实核查过程塞进正文、虚构作者自我纠错或未来计划；小红书同时检查正文与末卡是否强加互动、金句或建议。自然陈述句结尾完全合格。
- 错字与格式：同音词、重复字、的地得、术语、版本号、数字单位、中英文标点、标题层级、代码围栏、引用链接、表格列与图片说明。编号要求只针对博客和微信章节，代码中的 # 与小红书发布标题不套用章节编号。
- 知识：逐项找数字、版本/API、因果、绝对结论、引文、时效规则与代码行为。历史博客只能证明作者语气，不能证明知识正确。价格/政策/产品现状等必须检索当前权威来源，技术查官方文档、源码或论文；具体来源和日期记录在 fact_checks；正文保留必要的引用、适用口径和影响结论的限制，不复述编辑核查流程。必要时做最小可复现实验；没有执行就不能写“已验证”。
...
```json
{
  ...
  "reviews": {
    "polished": {
      "voice": "原稿短片段与位置、实际保留的表达特征，必要重写及其理由",
